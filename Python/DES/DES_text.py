...
p_box = [15, 6, 19, 20, 28, 11, 27, 16, 0, 14, 22, 25, 4, 17, 30, 9, 1, 7, 23, 13, 31, 26, 2, 8, 18, 12, 29, 5, 21, 10,
         3, 24]
...
def get_encryption_key(key_char):
    key = BitVector(textstring=key_char)
...
FILEKEY = open(sys.argv[3], 'r')
key_char = FILEKEY.read()
key_bit = get_encryption_key(key_char)
if sys.argv[1] == "-e":
    round_keys = generate_round_keys(key_bit)
else:
    round_keys = generate_round_keys(key_bit)
    round_keys.reverse()
...
while(bv.more_to_read):
    ...
    if length > 0:
        if length < 64:
            bitvec.pad_from_right(64 - length)
        [LE, RE] = bitvec.divide_into_two()
        for rkey in round_keys:
            temp_RE = RE
            newRE = RE.permute(expansion_permutation)
            out_xor = newRE ^ rkey
            newRE = substitute(out_xor)
            newRE = newRE.permute(p_box)
            LE = LE ^ newRE
            RE = LE
            LE = temp_RE
        RE_hex = RE.get_bitvector_in_hex()
        LE_hex = LE.get_bitvector_in_hex()
        FILEOUT.write(RE_hex)
        FILEOUT.write(LE_hex)
