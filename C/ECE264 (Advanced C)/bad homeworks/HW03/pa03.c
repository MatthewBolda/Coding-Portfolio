...
{
 	// number for how large malloc should be
	double * * data = malloc(sizeof(* data) * nrow);
 	for (int i =0; i < nrow;i++)
	{ 
		data [i] = malloc(sizeof(data) * ncol);
	}
...
{
	// free the inner memory using the same loop as initialized
	for (int i=0; i < nrow; i++)
	{ 
		free (data[i]);
	}
	// free the original malloc
	free(data);
		...
		int * cluster)
{
	...
	*/
	int nPoint;
	int nCentroid = kval * dim;
	int sameCent = 0;
	double dAve;
	
	while(nCentroid != sameCent)
	{
		//this loops the data for which centroid is closest
		for(int i=0; i<nval;i++)
		{
		cluster[i] = closestCentroid(kval, dim, data[i],centroid);
		}
	
		//loop for all the centroids
		for(int i=0; i<kval ;i++)
		{
			//loop for all the dimensions of data
			for(int a=0; a < dim; a++)
			{
			// remove old values for the averages
			dAve = 0;
			nPoint = 0;
