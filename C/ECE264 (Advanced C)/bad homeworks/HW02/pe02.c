...
for (int i=0; i < dim;i++)
{
	double dist = datapoint[i] -centroid[i];
	dist = dist *dist;
	sum +=dist;
}
...
{
  double sum;
  //fine the initial distance
  sum = distance2(data,centroid[0],dim);
  double mindist = sum;
  
  int mindex = 0; 
  // loop through all the centroids to find closest one
  for (int i = 1;i < kval; i++){
	double pmin = distance2(data,centroid[i],dim);
	// if the new distance is smaller, set the mindex to closer index
