...
int main(int argc, char ** argv) {
	// TODO 1. check the arguments, print "Wrong arguments\n" if anything fails DONEEEEEEEEEEEEEEEEEEEEEEEEEEE
	...
	*/
	StudentDatabase * db = ReadDatabase(argv[1]);
	if( db == NULL){
	...
	//test to see if argv[2] is a
	if(strcmp(argv[2], "-a") == 0){
	//	SortDatabaseByAge()
		SortDatabaseByAge(db);
		
		int test = WriteDatabase(argv[3], db);
		if(test == 1){
			printf("Write to db\n");
		}
		else{
	...
	// if it is neither a nor n do failure
	else{
		printf("Wrong arguments\n");
		Close(db);
		return EXIT_FAILURE;
