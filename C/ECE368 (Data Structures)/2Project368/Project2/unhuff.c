...
LinkedList * _data_linked_list(unsigned long long * count_array){
	LinkedList * linked_list = malloc(sizeof(LinkedList)); //this data has been free'd
	int size = 0;										   // using _free_list
...
int main(int argc, char **argv){
	...
	*/
	...
	//free(count_array);
	//_free_list(tester->head);
	//free(tester);
	//_free_list(table_head -> head);
	//free(table_head);
	//_free_tree(tree_top);
	//free(test_heap -> array);
	//free(test_heap);
	//fclose(fp_in);
	//free(filename);
	//fclose(unhuff_file);
	//*/
	printf("please no seg fault");
	return EXIT_SUCCESS;
}
