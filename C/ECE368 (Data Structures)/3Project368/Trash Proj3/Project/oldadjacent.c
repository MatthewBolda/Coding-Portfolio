...
//	return hash;
//}
//Node* _search_in_hash(Node** hashArray, int key){
...
void _get_hash_map(FILE* fp, Node** harray,int size){
	...
	while(index < size){
		...
		node -> connected = malloc(sizeof(int)*(size));//used to be 100000
		node -> distance = -1;
		node -> visited = 0;
...
//fill the hash with connected edges
void _fill_connected_hash(FILE* fp, Node** harray){
	int starter_key = _get_next_num_from_file(fp);
	int connected_key = _get_next_num_from_file(fp);

	Node* temp1 = harray[starter_key];
	Node* temp2 = harray[connected_key];
	
	temp1-> connected[temp1->index_connected] = connected_key;
	temp1-> index_connected++;
	temp2-> connected[temp2->index_connected] = starter_key;
...
void _fill_connected(FILE* fp, Head* header){
	...
	while(temp1 -> name != starter || temp2 -> name != connected){			//queue makes this faster
...
void _free_everything_hash(Node** harray,int size){
	int index = 0;
	while(index < size){
		Node* temp = harray[index];
		free(temp->connected);
		free(temp);
	}
}
...
void _get_dist_table(Head* header, Queue* queue){//,Node* destination){//
	...
	while(!isQueueEmpty(queue)){
		...
		Node* temp = header-> head;
		while(temp->name != checker){						//queue makes this faster
			temp = temp-> next;
		}
		...
		if (temp->visited == 0){
			...
			for(i=0; i< (temp->index_connected); i++){
				...
				while(next_node -> name != temp_name){
					next_node = next_node -> next;			//queue makes this faster
				}
...
//function to implement my version of dijkstras algorithm
void _get_dist_hash(Node** harray, Queue* queue){//,Node* destination){//
	//need head, queue
	while(!isQueueEmpty(queue)){
		int checker = _delete_from_queue(queue);
...
void _print_result(Node* destination){
...
//function to reset the distance and visited flags for next query
void _clear_distances_hash(Node** harray, int size){
	int index = 0;
	while(index <= size){
		harray[index]-> prev = NULL;
		harray[index]-> visited = 0;
		harray[index]-> distance = -1;
		index++;
	}
}
...
int main(int argc, char ** argv){
	...
	//if (cons){}
	//*/
	_get_hash_map(fp, hashArray,num);
	//_get_list_nodes(fp, header);		//this wouldnt be needed/ changed
	
	int i;
	for(i = 0; i<(cons); i++){
		_fill_connected_hash(fp,hashArray);
		//_fill_connected(fp, header);	//needed to be changed
	}
	fclose(fp);
	fp = fopen(argv[2], "r");
	...
	for(i = 0; i<num_of_queries; i++){
		...
		/*
		while(temp2 -> next != NULL && end_found != 1){			//hash makes this faster
		...
		while(start_found != 1 && temp2-> next != NULL){
			if(temp2 -> name == source){					//hash makes this faster
				start_found = 1;
			}
			temp2 = temp2 -> next;
		}
		*/
		if(end_found == 1 && start_found == 1){
			...
			//destination = hashArray[destination]
			_get_dist_hash(hashArray, queue);					//TODO function
			if(destination_node -> distance == -1){
				...
				_print_result(destination_node);
			}
			//_clear_distances(header);
			_clear_distances_hash(hashArray, num);					//TODO function
		}
		else{
			printf("INF\n");
			printf("%d %d", source, destination);
	...
	_free_everything_hash(hashArray, num);						//TODO function
